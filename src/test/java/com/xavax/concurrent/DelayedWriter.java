...
public class DelayedWriter<T> extends Thread {
	 ...
	 * @param result   the promised result.
	 */
	public static <T> void delayedWrite(final long delay, final Promise<T> promise, final T result) {
		DelayedWriter<T> writer = new DelayedWriter<>(delay, promise, result);
		writer.start();
	}
}